...
from exploration.ucb import UCBExplorer
...
def train_ucb(
    ...
    experiment_name: str = "drrmax_ucb"
):
    """
    Train DDPG agent using UCB exploration strategy.
    """
    ...
    explorer = UCBExplorer(num_items=env.num_items, c=c)
    
    rewards_history = []
    actor_losses = []
    critic_losses = []
    
    print(f"Starting UCB DDPG Training for {num_episodes} episodes (c={c})...")
    for episode in range(1, num_episodes + 1):
        ...
        while not done:
            # Select action using UCB Explorer
        ...
        if episode % 10 == 0:
            ...
            print(
                ...
                f"Total UCB Steps: {explorer.total_steps} | "
...
if __name__ == "__main__":
    ...
    parser.add_argument('--c', type=float, default=1.0, help='UCB exploration bonus coefficient')
    args = parser.parse_args()
    
    train_ucb(num_episodes=args.episodes, batch_size=args.batch_size, c=args.c)
